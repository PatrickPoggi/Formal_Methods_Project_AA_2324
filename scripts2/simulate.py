...
class GridPathApp:
    def __init__(self, master, grid_height, grid_width):
        ...
        """
        self.master = master
        self.master.title(f"Draw Drone paths")
    ...
    for i in range(grid_height):
        ...
        for j in range(grid_width):
            if (i, j) in all_selected_positions:
                btn = tk.Button(master, text='', width=3, height=1, state='disabled', bg=color_map[(i, j)])
            elif current_phase == 'Exits Positions' and not (i == 0 or i == grid_height-1 or j == 0 or j == grid_width-1):
                btn = tk.Button(master, text='', width=3, height=1, state='disabled', bg='lightgray')
            else:
                btn = tk.Button(master, text='', width=3, height=1, command=lambda i=i, j=j: on_click(i, j))
            btn.grid(row=i, column=j)
            row.append(btn)
        buttons.append(row)
...
def main():
    ...
    if args.multiple_queries:
        # manage multilple queries
        queries = []
        temp_files = []
        # output_traces_dir = './Traces/'
        ...
        for query in queries:
            # Use a temporary file to store the modified model, 
            # Necessary for concurrent access to file by verify_query function
        ...
        for i, query in enumerate(queries):
            ...
            results[i] = '\n'.join(results[i])
            # results[i] = '\n'.join([line for line in results[i] if 'satisfied' in line.lower()])
            formatted_query = query.replace('\n', '')
            print(f'Query: {formatted_query}, Result: {results[i]}')
            # Find the strings ' -- Formula is satisfied.' or ' -- Formula is NOT satisfied.' inside the stings of results[i], splitting the string by '\n'
            # pattern = r'((.*Formula is satisfied.)|(.*Formula is NOT satisfied.))'
            # results[i] = re.match(pattern, results[i]).group(0)
            # results[i] = ''.join([line for line in results[i] if 'satisfied' in line.lower()])
            with open(os.path.join(results_output_directory_path, 'results.csv'), 'a') as f:
                f.write(f'{formatted_query}, {results[i]}\n')
        # Delete all temporary files (also the traces...)
        for temp_file in temp_files:
            # print(f"Deleting {temp_file}")
            xml_path = temp_file                    # ends with .xml
